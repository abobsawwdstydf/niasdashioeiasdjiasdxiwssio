...
export default function AuthPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Validate age (must be > 5 years old)
        if (birthday) {
          ...
          if (actualAge <= 5) {
            setError('Вам должно быть больше 5 лет для регистрации');
            setIsSubmitting(false);
            return;
          }
        }
        
        await register(username, displayName || username, password, bio, birthday || undefined);
      }
  ...
  return (
    ...
    >
          ...
          <form onSubmit={handleSubmit} className="space-y-4" autoComplete="off">
            <div>
              ...
              {!isLogin && usernameStatus === 'available' && (
                <p className="text-xs text-emerald-400 mt-1">✓ Username свободен</p>
              )}
              {!isLogin && usernameStatus === 'taken' && (
                <p className="text-xs text-red-400 mt-1">✗ Username занят</p>
              )}
            </div>
      ...
      {/* QR Auth Modal - mode='login' for non-logged-in users */}
      <QRAuthModal isOpen={showQRAuth} onClose={() => setShowQRAuth(false)} mode="login" />
    </motion.div>
  );
}
